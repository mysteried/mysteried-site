...
if (titleEl && STAGE.title) {
    ...
    document.title = `${STAGE.title} – ミステリアリティ`; // ページタブタイトルも更新
}
...
async function checkGeoOnce() {
    ...
    try {
        ...
        const msg = err.code === 1
            ? '位置情報の許可が必要です'
            : (err.code === 2 ? '取得に失敗しました' : 'タイムアウトしました');
...
function handlePos(p, isMock) {
    ...
    const d = distanceMeters(
        ...
        { lat: STAGE.target.lat, lng: STAGE.target.lng }
    );
    const inRange = d <= STAGE.target.radius_m;
    geoOK = inRange;
